...
export function getDBName(
  ...
  const sanitizedBackend = sanitize(backend);
  const sanitizedModel = sanitize(modelIdentifier);

  return `sonar/${sanitizedVault}/${sanitizedBackend}/${sanitizedModel}`;
}
...
export class MetadataStore extends WithLogging {
  ...
  static async initialize(
    ...
    const dbName = getDBName(vaultName, backend, embeddingModel);
  ...
  async getChunksByIds(ids: string[]): Promise<Map<string, ChunkMetadata>> {
    if (this.metadataCache) {
      const result = new Map<string, ChunkMetadata>();
      for (const id of ids) {
        const chunk = this.metadataCache.get(id);
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = () =>
        reject(new Error('Failed to get chunks by IDs'));
    });
  }
