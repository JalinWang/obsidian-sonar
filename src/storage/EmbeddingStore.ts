...
export class EmbeddingStore extends WithLogging {
  ...
  private embeddingsCacheMap: Map<string, number[]> | null = null;
  ...
  private invalidateCache(): void {
    ...
    this.embeddingsCacheMap = null;
  }

  async getEmbedding(id: string): Promise<number[] | null> {
    if (this.embeddingsCacheMap) {
      return this.embeddingsCacheMap.get(id) ?? null;
    }
  ...
  async getAllEmbeddings(): Promise<EmbeddingData[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        ...
        this.embeddingsCacheMap = new Map(
          this.embeddingsCache.map(e => [e.id, e.embedding])
        );
