import { ZvecEmbeddingStore } from '../storage/ZvecEmbeddingStore';
import { EmbeddingStore } from '../storage/EmbeddingStore';
import { MetadataStore, type ChunkMetadata } from '../storage/MetadataStore';
...
function matchesModality(
  ...
  const fileIsImage = isImageFilePath(filePath);
...
export class EmbeddingSearch extends WithLogging {
  ...
  private async searchChunks(
    ...
    const results = await this.searchByVectorInternal(
      queryEmbedding,
      type,
      options
    );
    ...
    this.log(
      `[${mode}] query="${query}" type=${type} embed=${tEmbed - t0}ms search=${tTotal - tEmbed}ms total=${tTotal - t0}ms`
    );
    return results;
  }
  ...
  private async searchChunksByVector(
    ...
    const mode = this.configManager.get('vectorSearchMode');
    this.log(`[${mode}] vector search type=${type} search=${tTotal - t0}ms`);
    return results;
  }
  ...
  private async searchByVectorInternal(
    ...
    const mode = this.configManager.get('vectorSearchMode');
    if (mode === 'bf') {
      return this.searchBFByVector(queryEmbedding, type, options);
    }
    return this.searchZvecByVector(queryEmbedding, type, options);
  }
  ...
  private async searchZvecByVector(
    ...
    const titleFilePaths: string[] = [];
    for (const { id } of rawResults) {
      if (ChunkId.isTitle(id)) {
        titleFilePaths.push(ChunkId.getFilePath(id));
      } else {
        contentIds.push(id);
      }
    }

    const metadataById = await this.metadataStore.getChunksByIds(contentIds);

    const metadataByFilePath = new Map<string, ChunkMetadata>();
    if (titleFilePaths.length > 0) {
      const uniquePaths = [...new Set(titleFilePaths)];
      const fetches = uniquePaths.map(async fp => {
        const chunks = await this.metadataStore.getChunksByFile(fp);
        if (chunks.length > 0) metadataByFilePath.set(fp, chunks[0]);
      });
  ...
  private async searchBFByVector(
    ...
    const allEmbeddings = await this.idbEmbeddingStore.getAllEmbeddings();
